...
{     
  ...
  f_unlink (REC_WAVE_NAME_1);
  f_unlink (REC_WAVE_NAME_2);
  f_unlink (REC_WAVE_NAME_3);
  f_unlink (REC_WAVE_NAME_4);
  f_unlink (REC_WAVE_NAME_5);
  f_unlink (REC_WAVE_NAME_6);
  f_unlink (REC_WAVE_NAME_7);
  f_unlink (REC_WAVE_NAME_8);
  f_unlink (REC_WAVE_NAME_9);
  f_unlink (REC_WAVE_NAME_10);
  f_unlink (REC_WAVE_NAME_11);
  f_unlink (REC_WAVE_NAME_12);
  f_unlink (REC_WAVE_NAME_13);
  f_unlink (REC_WAVE_NAME_14);
  f_unlink (REC_WAVE_NAME_15);


  //if (strcmp(NOMBRE, REC_WAVE_NAME)==0){
  if (indice==0){
  ...
  { 
    ...
    if (TimeRecBase <= DEFAULT_TIME_REC)
    {
      ...
      /* User button pressed */
      if (CmdIndex != CMD_RECORD)//acá tengo que modificar algo?
      {
        /* Stop Audio Recording */
        WaveRecorderStop();
        /* Switch Command Index to Play */
        CmdIndex = CMD_STOP;//ACA CAMBIA EL ESTADO DE GRABACION AL DE PLAY QUE DESP USA EL MAIN.C linea 223
        /* Toggoling LED6 to signal Play */
        LEDsState = LED4_TOGGLE; //se enciende el led azul cuando se pone en play
        break;
      }
    }
  ...
  if (indice==1){
    /* Open the file to write on it */
    if ((AppliState == APPLICATION_IDLE) || (f_open(&WavFile, REC_WAVE_NAME_1, FA_CREATE_ALWAYS | FA_WRITE) != FR_OK))
    {
      while(1)
      {
        /* Toggle LED5 in infinite loop to signal that: USB Flash Disk is not connected/removed
  ...
  if (indice==2){
      /* Open the file to write on it */
      if ((AppliState == APPLICATION_IDLE) || (f_open(&WavFile, REC_WAVE_NAME_2, FA_CREATE_ALWAYS | FA_WRITE) != FR_OK))
      {
        while(1)
        {
          /* Toggle LED5 in infinite loop to signal that: USB Flash Disk is not connected/removed
  ...
  if (indice==3){
    /* Open the file to write on it */
    if ((AppliState == APPLICATION_IDLE) || (f_open(&WavFile, REC_WAVE_NAME_3, FA_CREATE_ALWAYS | FA_WRITE) != FR_OK))
    {
      while(1)
      {
        /* Toggle LED5 in infinite loop to signal that: USB Flash Disk is not connected/removed
  ...
  if (indice==4){
     /* Open the file to write on it */
     if ((AppliState == APPLICATION_IDLE) || (f_open(&WavFile, REC_WAVE_NAME_4, FA_CREATE_ALWAYS | FA_WRITE) != FR_OK))
     {
       while(1)
       {
         /* Toggle LED5 in infinite loop to signal that: USB Flash Disk is not connected/removed
  ...
  if (indice==5){
     /* Open the file to write on it */
     if ((AppliState == APPLICATION_IDLE) || (f_open(&WavFile, REC_WAVE_NAME_5, FA_CREATE_ALWAYS | FA_WRITE) != FR_OK))
     {
       while(1)
       {
         /* Toggle LED5 in infinite loop to signal that: USB Flash Disk is not connected/removed
  ...
  if (indice==6){
     /* Open the file to write on it */
     if ((AppliState == APPLICATION_IDLE) || (f_open(&WavFile, REC_WAVE_NAME_6, FA_CREATE_ALWAYS | FA_WRITE) != FR_OK))
     {
       while(1)
       {
         /* Toggle LED5 in infinite loop to signal that: USB Flash Disk is not connected/removed
  ...
  if (indice==7){
     /* Open the file to write on it */
     if ((AppliState == APPLICATION_IDLE) || (f_open(&WavFile, REC_WAVE_NAME_7, FA_CREATE_ALWAYS | FA_WRITE) != FR_OK))
     {
       while(1)
       {
         /* Toggle LED5 in infinite loop to signal that: USB Flash Disk is not connected/removed
  ...
  if (indice==8){
     /* Open the file to write on it */
     if ((AppliState == APPLICATION_IDLE) || (f_open(&WavFile, REC_WAVE_NAME_8, FA_CREATE_ALWAYS | FA_WRITE) != FR_OK))
     {
       while(1)
       {
         /* Toggle LED5 in infinite loop to signal that: USB Flash Disk is not connected/removed
  ...
  if (indice==9){
     /* Open the file to write on it */
     if ((AppliState == APPLICATION_IDLE) || (f_open(&WavFile, REC_WAVE_NAME_9, FA_CREATE_ALWAYS | FA_WRITE) != FR_OK))
     {
       while(1)
       {
         /* Toggle LED5 in infinite loop to signal that: USB Flash Disk is not connected/removed
  ...
  if (indice==10){
     /* Open the file to write on it */
     if ((AppliState == APPLICATION_IDLE) || (f_open(&WavFile, REC_WAVE_NAME_10, FA_CREATE_ALWAYS | FA_WRITE) != FR_OK))
     {
       while(1)
       {
         /* Toggle LED5 in infinite loop to signal that: USB Flash Disk is not connected/removed
  ...
  if (indice==11){
     /* Open the file to write on it */
     if ((AppliState == APPLICATION_IDLE) || (f_open(&WavFile, REC_WAVE_NAME_11, FA_CREATE_ALWAYS | FA_WRITE) != FR_OK))
     {
       while(1)
       {
         /* Toggle LED5 in infinite loop to signal that: USB Flash Disk is not connected/removed
  ...
  if (indice==12){
     /* Open the file to write on it */
     if ((AppliState == APPLICATION_IDLE) || (f_open(&WavFile, REC_WAVE_NAME_12, FA_CREATE_ALWAYS | FA_WRITE) != FR_OK))
     {
       while(1)
       {
         /* Toggle LED5 in infinite loop to signal that: USB Flash Disk is not connected/removed
  ...
  if (indice==13){
     /* Open the file to write on it */
     if ((AppliState == APPLICATION_IDLE) || (f_open(&WavFile, REC_WAVE_NAME_13, FA_CREATE_ALWAYS | FA_WRITE) != FR_OK))
     {
       while(1)
       {
         /* Toggle LED5 in infinite loop to signal that: USB Flash Disk is not connected/removed
  ...
  if (indice==14){
     /* Open the file to write on it */
     if ((AppliState == APPLICATION_IDLE) || (f_open(&WavFile, REC_WAVE_NAME_14, FA_CREATE_ALWAYS | FA_WRITE) != FR_OK))
     {
       while(1)
       {
         /* Toggle LED5 in infinite loop to signal that: USB Flash Disk is not connected/removed
  ...
  if (indice==15){
     /* Open the file to write on it */
     if ((AppliState == APPLICATION_IDLE) || (f_open(&WavFile, REC_WAVE_NAME_15, FA_CREATE_ALWAYS | FA_WRITE) != FR_OK))
     {
       while(1)
       {
         /* Toggle LED5 in infinite loop to signal that: USB Flash Disk is not connected/removed
